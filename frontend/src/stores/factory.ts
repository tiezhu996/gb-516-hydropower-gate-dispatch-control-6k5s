  ...
  return defineStore(id, {
    ...
    actions: {
      async load(path: string, search = '') { this.loading = true; this.error = ''; try { const result = await request<DomainRecord[]>(`/${path}?page=1&pageSize=20&search=${encodeURIComponent(search)}`); this.items = result.data; this.meta = result.meta || { page: 1, pageSize: 20, total: result.data.length }; } catch (error) { this.error = error instanceof Error ? error.message : String(error); } finally { this.loading = false; } },
			...
			async updateRecord(path: string, id: number, input: Partial<DomainRecord>) { this.loading = true; this.error = ''; try { await request<DomainRecord>(`/${path}/${id}`, { method: 'PUT', body: JSON.stringify(input) }); await this.load(path); } catch (error) { this.error = error instanceof Error ? error.message : String(error); } finally { this.loading = false; } },
			async transition(path: string, item: DomainRecord, status: string, reason: string) { this.loading = true; this.error = ''; try { await request<DomainRecord>(`/${path}/${item.id}/transition`, { method: 'POST', body: JSON.stringify({ status, expectedVersion: item.version, reason }) }); } catch (error) { this.error = error instanceof Error ? error.message : String(error); } finally { await this.load(path); this.loading = false; } },
    },
  });
}
