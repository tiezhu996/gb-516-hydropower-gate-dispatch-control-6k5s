...
- 执行回执必须登记现场实测闸位与观测时间。确认时服务端重新读取指令和闸门，仅当①实测值等于指令目标闸位、②观测时间严格晚于指令开始执行时间（`executedAt`）、③确认瞬间闸门当前状态与实测值相同三项全部满足，指令才完成；任一不符回执保持 `pending`、指令保持 `executing`，回执写入 `verifyStatus/verifyDetail` 说明具体原因，可补正后重试。
- 确认接口只读闸门、绝不改写闸门；闸位由现场设备经闸门接口反馈。失败回执才会中止指令并闭锁闸门。
...
- 指令开始执行时闸门原子进入 `moving`；回执三项校验全部通过才完成指令（闸门必须已由现场反馈至目标状态，确认接口不写闸门），失败回执同时中止指令并闭锁闸门。
